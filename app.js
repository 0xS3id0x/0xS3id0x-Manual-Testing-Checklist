...
════════════════════════════════════════════════════════════ */
const VERSION = '1.1.0';
console.log(`🛡️  0xS3id0x Checklist v${VERSION} loaded`);
...
function applyFilter(filterValue) {
  const validFilters = ['all', 'critical', 'high', 'medium', 'low', 'done', 'undone'];
  activeFilter = validFilters.includes(filterValue) ? filterValue : 'all';
  
  document.querySelectorAll('.fbtn[data-f]').forEach(b => b.classList.remove('active'));
  const activeBtn = document.querySelector(`.fbtn[data-f="${activeFilter}"]`);
...
document.addEventListener('keydown', event => {
  ...
  if (event.key.toLowerCase() === 'r' && !typingInInput) {
    checked = {};
    save();
    render();
    return;
  }
  if (event.key.toLowerCase() === 'f' && !typingInInput) {
    const filters = ['all', 'critical', 'high', 'medium', 'low', 'done', 'undone'];
...
function boot() {
  migrateCheckedKeys();
  save();
  // Use applyFilter to ensure UI + state sync on load (fixes empty-on-load bug)
  applyFilter('all');
  console.log('✅ Checklist ready | Filter:', activeFilter, '| Sections:', DATA.length);
}
